...
class CalendarFetcher {
    ...
    suspend fun queryCalendarData(context: Context): List<CalendarData> =
    ...
    private fun getCalendarEvents(context: Context, widgetId: String): List<CalendarEvent> {
        ...
        val selectionArgs = arrayOf(endTime.toString(), startTime.toString(), endTime.toString())
        ...
        context.contentResolver.query(uri, projection, selection, selectionArgs, sortOrder)?.use {
            ...
            while (cursor.moveToNext()) {
                ...
                val allDay =
                    cursor.safeGet(CalendarContract.Instances.ALL_DAY, Cursor::getInt)?.let {
                        it != 0
                    }
                val actualStartTime =
    ...
    private fun parseAndTransformCalendarItems(
        parsedCalendarEvents: List<CalendarEvent>
    ): List<CalendarViewItem> {
        val timeZone = TimeZone.getDefault()
        // Get the current date at the start of the day
        val currentDayStartTime =
        ...
        return parsedCalendarEvents
            .flatMap { event ->
                val adjustedEvent =
                    if (event.startTimeInMillis < currentDayStartTime) {
            ...
            .flatMap { (date, events) ->
                val sortedEvents =
                    events.sortedWith(compareBy({ !it.isAllDay }, { it.startTimeInMillis }))
    ...
    private fun splitMultiDayEvent(event: CalendarEvent): List<CalendarEvent> {
        ...
        while (startCalendar.before(endCalendar)) {
            ...
            val eventEndTime: Long =
                if (nextDayStartCalendar.before(endCalendar)) nextDayStartCalendar.timeInMillis
                else event.endTimeInMillis
